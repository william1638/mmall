...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> selectQuestion(String username){
        ...
        return ServerResponse.createByErrorMessage("用户没有找回密码问题");
    }
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew,User user){
        //防止横向越权,要校验一下这个用户的旧密码,一定要指定这个用户,因为我们会查询一个count(1)，如果不指定id,那么结果就是true count>0
    ...
    public ServerResponse<User> updateInformation(User user){
        //username是不能被更新的
        //email也要进行一个校验,校验新的email是不是已经存在的email
        int resultCount = userMapper.checkEmailByUserId(user.getEmail(),user.getId());
        if(resultCount > 0 ){
            return ServerResponse.createByErrorMessage("email已经存在,请换一个");
        }
